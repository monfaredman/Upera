<template>
  <b-modal
    ...
    hide-footer
    size="lg"
    modal-class="modal-download-link credit"
    title="افزایش موجودی"
  >
      ...
      <div class="download-links-body download-links-body-header">
        <div class="download-links-items download-links-items-credit">
          <div class="tenor tenor-m">
            <nuxt-link
              class="btn btn-light text-secondary show-mobile text-center btn-block mb-3"
              to="/payments"
            ...
            <div
              class="mt-sm-5 d-flex align-items-center justify-content-between border-bottom-md-gray pb-md-5"
            >
              <div
                class="d-flex align-items-center justify-content-between w-sm-100"
              >
              ...
              <nuxt-link
                class="btn btn-light text-secondary hide-mobile"
                to="/payments"
                @click.native="hideModal()"
              >
                پرداخت&zwnj;های قبلی
            ...
            <div
              class="mt-5 form-group d-md-flex align-items-center justify-content-between mb-md-5 form-inline"
            >
              <label>مبلغ مورد نظر خود را وارد کنید</label>
              <input
                v-model="charge"
                type="text"
                class="form-control form-control-lg form-control-bk text-center mb-md-0"
                autofocus
              />
            </div>
            <div class="mb-3 mb-md-6 d-flex overflow-x-auto overflow-buttons">
              <button
                class="btn btn-light font-weight-bold text-nowrap"
                @click="charge = 10000"
              >
                ۱۰.۰۰۰ تومان
              </button>
...
export default {
  ...
  mounted() {
    if (this.$auth.loggedIn && !this.prewallet) {
      ...
      this.$axios.post('/get/my_wallet', {}).then(
        (res) => {
  ...
  methods: {
    BUY() {
      ...
      this.$axios
        .post('/get/charge_wallet', {
          amount: this.charge,
          payment_method: this.payment_method,
          callback_url:
            location.origin + '/callback?mobile=' + this.checkuser.mobile,
        })
        .then(
          (res) => {
            this.message = false
