  ...
  <div>
    <div v-if="!itemmenu.includes($route.name)" class="d-flex header-mobile d-md-none">
      ...
      <div v-if="$config.envname=='upera'" class="header-search">
        <a href="/kids" class="show-mobile header-link d-flex flex-column align-items-center justify-content-center text-nowrap">
      ...
      <div id="trigger-menu-wrapper" class="container-fluid trigger-menu-wrapper" :class="{'trigger-menu-wrapper2':bannerNav}">
        ...
        >
          <div class="d-flex align-items-center h-full">
            ...
            <div class="d-none d-md-flex align-items-center h-full">
              <div class="category d-flex align-items-center h-full header-category-profile" :class="{ 'open': categoriesNav }" @mouseover="categoriesHover()" @mouseleave="categoriesleave()">
                <div class="header-links header-links-category d-md-flex align-items-center mr-md-2 mr-lg-3 h-full">
                  <nuxt-link to="/genres" class="d-flex align-items-center header-link">
              ...
              <div v-if="$config.envname=='upera'" class="category header-buttons d-flex align-items-center h-full header-category-profile">
                <div
          ...
          <div class="header-buttons d-flex align-items-center h-full">
            ...
            >
              <b-link id="popover-settings" class="d-flex align-items-center header-link">
            ...
            <div class="btn-app">
              <b-button v-if="!content_subscription && checkuser.subscription!=1 && !$auth.loggedIn && $route.name !=='login'" variant="second" class="d-flex align-items-center ml-2" @click="SHOW_MODAL_WITH_DIRECTDEBIT()">
                <i class="fa fa-credit-card ml-2 hide-mobile" />
                پرداخت خودکار
              </b-button>
              <b-button v-else-if="!content_subscription && checkuser.subscription!=1 && $auth.loggedIn" variant="second" class="d-flex align-items-center ml-2" @click="SHOW_MODAL_DIRECTDEBIT()">
                <i class="fa fa-credit-card ml-2 hide-mobile" />
                پرداخت خودکار
              </b-button>
              <b-button v-if="(content_subscription || checkuser.subscription==1) && !$auth.loggedIn" variant="second" class="d-flex align-items-center ml-2" @click="SHOW_MODAL_WITH_SUBSCRIPTION()">
                <i class="fa fa-credit-card ml-2 hide-mobile" />
                خرید اشتراک
              </b-button>
              <b-button v-else-if="(content_subscription || checkuser.subscription==1) && !checkuser.access" variant="second" class="d-flex align-items-center ml-2" @click="SHOW_MODAL_SUBSCRIPTION()">
                <i class="fa fa-credit-card ml-2 hide-mobile" />
                خرید اشتراک
              </b-button>
              <b-button v-else-if="(content_subscription || checkuser.subscription==1) && checkuser.access" variant="second" class="d-flex align-items-center ml-2" @click="SHOW_MODAL_SUBSCRIPTION()">
            ...
            <nuxt-link v-if="checkuser.show_app && $auth.loggedIn" to="/app" class="btn btn-main show-mobile">
              {{ $t('new.app') }}
            </nuxt-link>
            <b-button v-else variant="main" class="show-mobile" @click="SHOW_MODAL()">
              {{ $t('nav.login') }}
            </b-button>
            ...
            >
              ...
              <nuxt-link id="popover-profile" to="/profile" class="d-flex align-items-center btn btn-main">
                <i class="fa fa-user ml-2" />
                <span>{{ $t('setting.profile') }}</span>
                <i class="fa fa-angle-down mr-2" />
              </nuxt-link>
              <b-popover id="popover-d-profile" target="popover-profile" triggers="hover" placement="bottomleft" fallback-placement="flip" boundary-padding="1">
                <ul class="dropdown-menu show">
                  <li>
                    <ul class="nav flex-column">
                      <li class="nav-item">
                        <span class="nav-link nav-link-divider d-flex align-items-center flex-wrap">
                          ...
                          <div class="pr-2">
                            ...
                            <div v-show="checkuser.days_period_to_end" class="p-fs-small mt-2">
                      ...
                      <li class="nav-item nav-item-padding">
                        <b-button v-if="!content_subscription && checkuser.subscription!=1 && !$auth.loggedIn && $route.name !=='login'" variant="second" class="d-flex align-items-center justify-content-center w-full" @click="SHOW_MODAL_WITH_DIRECTDEBIT();$root.$emit('bv::hide::popover')">
                          <i class="fa fa-credit-card ml-2" />
                          پرداخت خودکار
                        </b-button>
                        <b-button v-else-if="!content_subscription && checkuser.subscription!=1 && $auth.loggedIn" variant="second" class="d-flex align-items-center justify-content-center w-full" @click="SHOW_MODAL_DIRECTDEBIT();$root.$emit('bv::hide::popover')">
                          <i class="fa fa-credit-card ml-2" />
                          پرداخت خودکار
                        </b-button>
                        <b-button v-if="(content_subscription || checkuser.subscription==1) && !$auth.loggedIn" variant="second" class="d-flex align-items-center justify-content-center w-full" @click="SHOW_MODAL_WITH_SUBSCRIPTION();$root.$emit('bv::hide::popover')">
                          <i class="fa fa-credit-card ml-2" />
                          خرید اشتراک
                        </b-button>
                        <b-button v-else-if="(content_subscription || checkuser.subscription==1) && !checkuser.access" variant="second" class="d-flex align-items-center justify-content-center w-full" @click="SHOW_MODAL_SUBSCRIPTION();$root.$emit('bv::hide::popover')">
                          <i class="fa fa-credit-card ml-2" />
                          خرید اشتراک
                        </b-button>
                        <b-button v-else-if="(content_subscription || checkuser.subscription==1) && checkuser.access" variant="second" class="d-flex align-items-center justify-content-center w-full" @click="SHOW_MODAL_SUBSCRIPTION();$root.$emit('bv::hide::popover')">
                      ...
                      <li class="nav-item nav-item-padding">
                        <div class="d-flex align-items-center justify-content-between flex-wrap">
                          ...
                          <button class="btn btn-secondary btn-sm" @click="SHOW_MODAL_CREDIT();$root.$emit('bv::hide::popover')">
                            {{ my_credit }}
                          </button>
                        </div>
                      </li>
                      <li class="nav-item">
                        <nuxt-link class="nav-link d-flex align-items-center flex-wrap" to="/profile">
                          <i class="fa fa-play-circle" />
                          <span class="pr-2">لیست‌های من</span>
                        </nuxt-link>
                      </li>
                      <li class="nav-item mt-2">
                        <nuxt-link class="nav-link d-flex align-items-center flex-wrap" to="/app">
                          <i class="icon-download" />
                          <span class="pr-2">دانلود اپلیکیشن</span>
                        </nuxt-link>
                      </li>
                      <li v-show="showpayments" class="nav-item mt-2">
                        <nuxt-link class="nav-link d-flex align-items-center flex-wrap" to="/payments">
                          <i class="fa fa-shopping-basket" />
                          <span class="pr-2">پرداخت ها</span>
                        </nuxt-link>
                      </li>
                      <li class="nav-item mt-2">
                        <b-link class="nav-link d-flex align-items-center flex-wrap" @click="logout()">
            ...
            <b-button v-else variant="main" class="hide-mobile" @click="SHOW_MODAL()">
              {{ $t('nav.login') }}
            </b-button>
