<template>
  <b-modal
    ...
    hide-footer
    size="lg"
    modal-class="credit subscription"
    title="خرید اشتراک"
  >
    ...
    <div
      v-else-if="checkuser.days_period_to_end"
    ...
    <div v-if="!planloading">
      <div v-for="(plan, index) in plans" :key="index">
        <div
          class="d-flex align-items-center justify-content-between font-weight-bold mb-3"
        >
          <span class="d-inline-block">اشتراک {{ plan.name_fa }}ه</span>
          <span class="d-inline-block position-relative"
            >{{ plan.toman.toLocaleString() }} <i class="icon-toman" />
            <span
              class="text-underline text-underline-primary-dark"
              style="width: 100%"
            />
          </span>
        </div>
        <div v-if="plan.discount" class="alert alert-success mb-3">
          اشتراک {{ plan.name_fa }}ه با {{ plan.discount.discount_percent }}%
          تخفیف<span v-if="plan.discount.discount_day">
            و {{ plan.discount.discount_day }} روز اشتراک هدیه</span
          >
        </div>
    ...
    <div
      v-if="!planloading && desc_subscription"
      class="pb-3 border-bottom-gray mb-3"
    >
      دقت کنید که تنها فیلم و سریال های موجود در آپرا پلاس را می توانید با خرید
      این اشتراک مشاهده نمایید<br /><a
        target="_blank"
        href="https://plus.upera.tv"
    ...
    <div v-if="!planloading" class="alert alert-warning mb-4">
      ...
      <span v-if="showfullrate"
        ><b>امکان دسترسی به فیلم ها و سریال ها بدون خرید اشتراک:</b><br />
        با اینترنت {{ operator_fullrate }} وارد شوید ، حجم مصرفی به جای خرید
        اشتراک تمام بها محاسبه می شود.<br
      /></span>
    </div>
    <!-- <div v-if="!planloading" v-lazy-load="description" /> -->

    <b-button
      v-if="!planloading && !$auth.loggedIn"
      variant="main"
      class="py-1 px-lg-4 btn-block"
      @click="SHOW_MODAL()"
    >
  ...
  watch: {
    show(val) {
      if (val !== null && this.show) {
        ...
        this.$axios.get('/get/app/plans').then(
          (res) => {
