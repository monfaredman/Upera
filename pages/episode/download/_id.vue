  ...
  async asyncData(context) {
    ...
    try {
      [episodeRes, seasonRes] = await Promise.all([
        ...
        context.app.$axios.get('/ghost/get/season/episode/' + context.params.id)
      ])
    } catch (e) {
      context.redirect('/')
      return e
    ...
    return { 
      ...
      title: 'دانلود ' + (
        (this.data.episode.season_number == 1)
          ? (this.ChooseLang(this.data.episode.series_name, this.data.episode.series_name_fa) + ',قسمت ' + this.data.episode.episode_number)
          : (this.ChooseLang(this.data.episode.series_name, this.data.episode.series_name_fa) + ',قسمت ' + this.data.episode.episode_number + ' فصل ' + this.data.episode.season_number)
      )
    }
  },
  mounted() {
    ...
    (async () => {
      ...
      this.$set(this.data.episode, 'owned_period_end', accessibilityData.owned_period_end)
    })().catch(error => {
