      ...
      <div class="row">
        <div v-for="(item,index) in data.casts" :key="index" class="col-4 col-xl-1 col-md-2 col-sm-3 mt-2 mt-lg-4">
          <nuxt-link :to="{ name: 'cast-id', params: { id: item.id }}" class="actor">
            <b-img v-bind="{fluidGrow: true,blank: true,blankColor: '#bbb',width: 141,height: 214,show:true}" :src="'https://thumb.upera.shop/thumb?w=141&h=214&q=100&a=c&src=https://cdn.upera.shop/s3/casts/'+item.image" :alt="item.name" />
