        ...
        >
          ...
          >
            <b-img
              ...
              :src="
                'https://thumb.upera.tv/thumb?w=141&h=214&q=100&a=c&src=https://cdn.upera.tv/s3/casts/' +
                item.image
              "
              :alt="item.name"
            />
            <!-- <img :src="'https://thumb.upera.tv/thumb?w=141&h=214&q=100&a=c&src=https://cdn.upera.tv/s3/casts/'+item.image" :alt="item.name"> -->
...
export default {
  ...
  async asyncData(context) {
    ...
    if (context.app.$auth.loggedIn) {
      res = await context.app.$axios.get('new_casts/1/ir/0')
    } else {
      res = await context.app.$axios.get('new_casts/1/ir/0')
    }
  ...
  head() {
    return { title: this.$t('new.iranianactors') }
  },
  methods: {
    ChooseLang(en, fa) {
    ...
    infiniteHandler($state) {
      ...
      this.$axios
        .get(apiurl, { params: { page: this.page + 1 } })
        .then((response) => {
