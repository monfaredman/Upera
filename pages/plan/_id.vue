<template>
  <div id="checkcontainer">
    <section id="slideshow" class="slideshow d-none d-md-block pt-5">
      <div>
        <div
          v-swiper:topSwiper="swiperOption"
          class="swiper-container app-slider"
          dir="rtl"
        >
    ...
    <div class="container faq-page">
      <div class="app_container_pricing mt-4 p-2 mt-md-4 mb-5">
        <header class="headline py-md-5">
          <h5 class="title font-weight-bold">انتخاب شیوه پرداخت</h5>
        </header>
          ...
          <div class="row">
            ...
            <div class="col-md-6 col-sm-12">
              <a
                ...
                @click.prevent="SHOW_MODAL_CREDIT()"
              >
                افزایش موجودی
        ...
        <div class="text-center">
          <h3 v-if="loading">لطفا منتظر بمانید...</h3>
...
export default {
  ...
  methods: {
    async get_wallet() {
      this.loading = true
      await this.$axios.post('/get/my_wallet', {}).then(
        (res) => {
          this.loading = false
          if (res.status === 200) {
            ...
            if (res.status === 200) {
              ...
              if (this.data && this.data.status == 'success') {
                if (this.method == 'credit') {
                  ...
                  setTimeout(() => {
                    this.$swal(
                      'از خرید اشتراک شما ممنونیم. اشتراک شما فعال شد. ' +
                        this.checkuser.days_period_to_end +
                        ' روز از اشتراک شما باقیمانده است',
                      {
                        icon: 'success',
                      }
                    ).then(() => {
