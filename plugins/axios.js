export default function (context) {
  ...
  // Language detection is now handled by plugins/i18n-detection.client.js
  // which runs before this plugin to ensure proper initialization

  context.app.i18n.onBeforeLanguageSwitch = (
    oldLocale,
    newLocale,
    isInitialSetup,
    context
  ...
  context.app.$axios.onError((error) => {
    ...
    if (error.response && error.response.status === 401) {
      context.app.$axios.get('/logout?2').then(
        (response) => {
